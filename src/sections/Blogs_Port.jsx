...
const handleBlogData = (index) =>{
  const dataProfile = blogs[index]
  setBlogData(dataProfile)
}
  return (
    <>
            ...
            <div className='grid grid-cols-3 gap-8 mx-3'>
                {blogs.map((data, index)=>(
                  <div key={index} className='flex flex-col gap-5'>
                      <NavLink onClick={()=>handleBlogData(index)} className='relative group overflow-hidden rounded-lg'>
                          <img src={data.img} alt="Blog_img" className='rounded-lg group-hover:scale-104 group-hover:rotate-2 duration-300 ease'/>
                          ...
                          <div className='absolute h-full bg-transparent w-full top-0 left-0 flex justify-center items-center'>
                            <span className='text-[#ff7a33] font-semibold text-md scale-y-0 group-hover:scale-y-100 duration-300 delay-55 origin-top flex items-center gap-2'>Click to Open <HiCursorClick/></span>
                          </div>
                      </NavLink>
                      <NavLink onClick={()=>handleBlogData(index)} className='py-2 hover:text-[#ff7a33] text-start duration-150 text-xl font-semibold'>
                          {data.title}
                      </NavLink>
